...
ADX_ENABLED    = TRADING_CONFIG["adx_enabled"]        # True
...
def close_trade(
    ...
    entry_ltp:     float,
    entry_fill:    float,
    adx_vals:      tuple,         # (adx, plus_di, minus_di)
    spot:          float,
    reason:        str,
    now:           datetime,
    fyers:         fyersModel.FyersModel,
...
def run(initial_capital: float):
    ...
    log.info(f"  ADX filter   : {'ON  (period=' + str(TRADING_CONFIG['adx_period']) + ' min=' + str(TRADING_CONFIG['adx_min']) + ')' if ADX_ENABLED else 'OFF'}")
    ...
    active_adx_vals:  tuple             = (0.0, 0.0, 0.0)
    ...
    while True:
        try:
            ...
            # ── Look for new entry ─────────────────────────────────────────────
                ...
                # ADX momentum filter
                adx_ok, adx_val, pdi, ndi = sig_eng.get_adx(direction)
                if not adx_ok:
                    # get_adx already logged the skip reason
                    time.sleep(TRADING_CONFIG["poll_interval"])
                    continue
                ...
                active_adx_vals    = (adx_val, pdi, ndi)
