...
class OptionSelector:
    ...
    def _load(self):
        ...
        # Step 1: Read raw to detect actual column count
        raw = pd.read_csv(
        ...
        logger.info(f"[SymbolMaster] CSV columns detected: {n_cols} "
                    f"(expected {len(SYMBOL_MASTER_COLS)})")

        # Step 2: Assign column names — pad or trim if count differs
        if n_cols >= len(SYMBOL_MASTER_COLS):
            col_names = SYMBOL_MASTER_COLS + [
                f"extra_{i}" for i in range(n_cols - len(SYMBOL_MASTER_COLS))
            ]
        else:
            col_names = SYMBOL_MASTER_COLS[:n_cols]
        raw.columns = col_names

        # Step 3: Find NIFTY rows — search ALL columns for NIFTY
        # This handles Fyers changing column order in CSV updates
        nifty_mask = pd.Series([False] * len(raw), index=raw.index)
        for col in raw.columns:
            col_upper = raw[col].astype(str).str.strip().str.upper()
            nifty_mask |= (
                (col_upper == "NIFTY") |
                (col_upper == "NIFTY50") |
                (col_upper == "NIFTY 50")
        ...
        # Also try the expected underlying_symbol column directly
        if "underlying_symbol" in raw.columns:
            u = raw["underlying_symbol"].astype(str).str.strip().str.upper()
            nifty_mask |= (
                u.str.startswith("NIFTY") & ~u.str.contains("BANK")
            )
        df = raw[nifty_mask].copy()

        if len(df) == 0:
            # Log first few rows to help debug
            logger.error(
                "[SymbolMaster] 0 NIFTY rows found. "
                f"First row sample: {dict(raw.iloc[0][:8])}")
            raise ValueError(
                "Symbol master returned 0 NIFTY instruments. "
                "Check Fyers CSV format or network connection.")

        # Step 4: Parse columns we need
        if "expiry_date" in df.columns:
            df["expiry_date"] = pd.to_datetime(
                df["expiry_date"], errors="coerce").dt.date
